...
describe("DAG with WaveletMatrixEdges", () => {
  it("should work interchangeably with original Edges implementation", () => {
    ...
    const waveletDAG = new DAG<string, number>(new Nodes(), new WaveletMatrixEdges<number>());
    ...
    expect(originalDAG.edges.getValue(a1, b1)).toBe(waveletDAG.edges.getValue(a2, b2));
    expect(originalDAG.edges.getValue(a1, c1)).toBe(waveletDAG.edges.getValue(a2, c2));
    expect(originalDAG.edges.getValue(b1, d1)).toBe(waveletDAG.edges.getValue(b2, d2));
    expect(originalDAG.edges.getValue(c1, d1)).toBe(waveletDAG.edges.getValue(c2, d2));
    ...
    const originalPaths = [...originalDAG.findShortestPath({ from: a1, to: d1, costF: (e) => e.value })];
    const waveletPaths = [...waveletDAG.findShortestPath({ from: a2, to: d2, costF: (e) => e.value })];
  ...
  it("should maintain consistent behavior with complex DAG operations", () => {
    const waveletDAG = new DAG<string, string>(new Nodes(), new WaveletMatrixEdges<string>());
    
    // Create a more complex graph
    const nodes = ['root', 'a', 'b', 'c', 'd', 'leaf1', 'leaf2'].map(n => waveletDAG.nodes.add(n));
  ...
  it("should provide memory and serialization benefits", () => {
    ...
    const waveletDAG = new DAG<number, number>(new Nodes(), new WaveletMatrixEdges<number>());
    ...
    expect(originalSerialized.children.length).toBe(waveletSerialized.children.length);
    expect(originalSerialized.parent.length).toBe(waveletSerialized.parent.length);
    ...
    console.log('Original edges serialized size:', JSON.stringify(originalSerialized).length);
    console.log('Wavelet matrix edges serialized size:', JSON.stringify(waveletSerialized).length);
  });
});